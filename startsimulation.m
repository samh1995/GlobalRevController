...
IC.attEuler = [0; 0; 0]; % [roll; pitch; yaw] (radians)
...
for i = 0 : dt : endTime - dt
    ...
    [Control] = computedesiredacceleration(Control, Pose, Twist, recoveryStage); %updates control.acc
    [Control] = controllerrecovery(dt, Pose, Twist, Control, Hist, recoveryStage, i);
    
    % Propagate dynamics.
    options = odeset('RelTol',1e-3); %%tolerance
    [tODE,stateODE] = ode45(@(tODE, stateODE) dynamicsystem(tODE, stateODE, dt, Control.rpm, PropState.rpm),[i i+dt], Hist.states(:,end), options);
